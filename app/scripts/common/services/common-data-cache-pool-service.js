...
angular.module('GSH5').factory('DataCachePool', function () {
  ...
  var fetchItem = function(key) {
    ...
    var itemStr = window.localStorage.getItem('GS_FOODS_' + key), item;
    try {
  ...
  var pushData = function(key, dataKey, data) {
    ...
    var item = fetchItem(key), obj = {expired: Date.now() + ONE_DAY};
    obj[dataKey] = data;
    if(!item) {
      item = [obj];
    } else {
