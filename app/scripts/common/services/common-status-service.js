...
angular.module('EPBUY').factory('pageStatus', function ($location, $state, $compile) {

    /*
     * 调用方式：引入pageStatus和$scope, $stateParams依赖参数;
     * 在页面controller里声明 pageStatus.pageStatusInit($scope, $stateParams);
     * 如果页面初次加载需要遮罩，请在ion-view标签上加一个ng-show="hasResult"属性来控制页面隐藏；
     * 请求成功时调用loadSuccess方法;
     * 请求失败时网络报错，调用loadError方法;
     */

    var pageStatusInit = function (scope, stateParams) { //初始化页面
