...
angular.module('EPBUY').factory('AREA', function () {
    ...
    GetArea.prototype = {
        init: function (p, c, d) {
            ...
            if (p) {
                this.pName = p;
                for (var j in this.dat[p]) {
                    this.cDat.push(j);
                }

                if (c) {
                    this.cName = c;
                    this.dName = d || this.text[2];
                    for (var k = 0; k < this.dat[p][c].length; k++) {
                        this.dDat.push(this.dat[p][c][k]);
                    }
                } else {
                    this.cName = this.text[1];
                }
    ...
    /* 注释--------------------------
    初始:
    new GetArea(省,市,县) 初始按可选参数顺序设置属性值,默认初始pDat省列表
    属性:
    string:pName 省
    string:cName 市
    string:dName 县
    ...
    Array:dDat县列表
    方法:
    void:getCity(省), 改变省名和列表,其它属性置空
    void:getDistrict(市), 保留省信息,改变市名和县列表,县名置空
    */
